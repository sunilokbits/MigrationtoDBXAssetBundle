...
def _scheduler_tick_inner():
    ...
    for h in data.get("history", []):
        ...
        if gid:
            ...
            if grp:
                ...
                continue           # still running or just resolved
    ...
    for sch in data.get("schedules", []):
        ...
        # Guard: don't re-trigger if the pipeline is already running
        if group_id:
            grp = wfm.PIPELINE_GROUPS.get(group_id)
...
def _on_pipeline_done(group_id: str, status: str):
    ...
    Immediately updates matching 'running'/'started' history entries in the JSON file."""
    with _sch_file_lock:
        try:
