...
box(sl, dbr_x + Inches(0.15), nb_y, dbr_w - Inches(0.3), Inches(0.25),
    TEAL, "Auto-Generated Metadata Notebooks  (Standard PySpark + DLT Mode)", sz=8.5, fc=WHITE, bold=True)
...
pillars = [
    ...
    ("Scalability", TEAL, RGBColor(0xE0, 0xF2, 0xF1),
     ["Metadata-driven config", "DLT + Standard PySpark",
...
tech_items = [
    ...
    ("Delta Live Tables",       "Optional DLT mode: Auto Loader + expectations"),
...
txt(sl, Inches(0.2), Inches(7.2), Inches(13), Inches(0.2),
    "AI Workflow Manager  |  Metadata-Driven Medallion  |  Reconciliation + Execution Logging  |  DLT + PySpark  |  Per-Stage Rerun  |  deployconfig.json",
...
feature_card(sl2, col1_x, cy, col1_w, Inches(0.85),
    "3. Delta Live Tables (DLT) Pipeline Mode",
    ["New pipeline_mode: 'dlt' alongside 'standard' PySpark",
     "02_Meta_DLT_Pipeline — Auto Loader + DLT expectations",
     "DLT Orchestrator — Extract → DLT pipeline trigger (2 stages)",
...
feature_card(sl2, col2_x, cy2, col2_w, Inches(0.85),
    ...
    ["Stage progress bar with arrows in Pipeline Studio cards",
     "DLT mode: 2 stages (Extract → DLT Bronze+Silver)",
...
before_after = [
    ...
    ("Pipeline Modes", "Standard only", "Standard + DLT"),
