...
def sql_to_pyspark_auto(name: str, object_type: str, sql_code: str) -> str:
    ...
    def _col_expr(expr):
        """Convert a SQL column expression to F.expr() or F.col()."""
    ...
    def _agg_func(expr):
        """Map SQL aggregate to PySpark F.* expression."""
        expr = expr.strip()
        m = re.match(r"(?i)(SUM|COUNT|AVG|MIN|MAX|STDEV|VAR)\s*\(\s*(.*?)\s*\)\s*(?:AS\s+(\w+))?$", expr)
        if m:
            fn, arg, alias = m.group(1).upper(), m.group(2), m.group(3) or ""
            arg = _clean_name(arg)
            pf = {"SUM": "sum", "COUNT": "count", "AVG": "avg",
                  "MIN": "min", "MAX": "max", "STDEV": "stddev", "VAR": "variance"}.get(fn, "sum")
            base = f'F.{pf}("{arg}")'
            if "*" in arg:
    ...
    def _join_type(sql_kw):
        m = {"LEFT": "left", "RIGHT": "right", "OUTER": "outer",
             "INNER": "inner", "CROSS": "cross", "FULL": "outer"}.get(sql_kw.upper(), "inner")
        return m

    def _case_when(expr):
        """Convert CASE WHEN … THEN … ELSE … END to F.when chain."""
        ...
        for cond, val in whens:
            ...
            val  = val.strip().strip("'")
            if chain:
                chain += f'\n        .when(F.expr("{cond}"), F.lit("{val}"))'
            else:
                chain = f'F.when(F.expr("{cond}"), F.lit("{val}"))'
        if else_m:
            ev = else_m.group(1).strip().strip("'")
            chain += f'\n        .otherwise(F.lit("{ev}"))'
        return chain if chain else 'F.lit(None)'

    def _window_expr(expr):
        """Convert ROW_NUMBER/RANK OVER(PARTITION BY … ORDER BY …) to Window."""
        fn_m   = re.match(r"(?i)(ROW_NUMBER|RANK|DENSE_RANK|LAG|LEAD|NTILE|SUM|AVG|COUNT)\s*\(([^)]*)\)\s*OVER\s*\((.+)\)", expr, re.DOTALL)
        if not fn_m:
            return f'F.expr("{expr.replace(chr(34), chr(39))}")'
        fn, fn_args, over = fn_m.group(1).upper(), fn_m.group(2).strip(), fn_m.group(3)
        part_m = re.search(r"(?i)PARTITION\s+BY\s+([\w,\s\.\[\]]+?)(?=\s+ORDER|\s*$)", over)
        ord_m  = re.search(r"(?i)ORDER\s+BY\s+([\w,\s\.\[\]]+?)(?=\s*$)", over)
        parts  = [f'"{_clean_name(p.strip())}"' for p in part_m.group(1).split(",")] if part_m else []
        orders = [f'"{_clean_name(o.strip().split()[0])}"' for o in ord_m.group(1).split(",")] if ord_m else []
        w = "Window"
        if parts:  w += f'.partitionBy({", ".join(parts)})'
        if orders: w += f'.orderBy({", ".join(orders)})'
        pf_map = {"ROW_NUMBER": "row_number()", "RANK": "rank()", "DENSE_RANK": "dense_rank()",
                  "LAG": f"lag({fn_args})", "LEAD": f"lead({fn_args})",
                  "SUM": f"sum({fn_args})", "AVG": f"avg({fn_args})", "COUNT": "count(F.lit(1))"}
        return f'F.{pf_map.get(fn, fn.lower()+"()")}.over({w})'
    ...
    def _parse_ctes(sql):
        ...
        if re.match(r"(?i)\s*WITH\b", sql):
            # iteratively pull CTEs
            pattern = re.compile(
                r"(?i)\bWITH\b\s+(\w+)\s+AS\s*\(", re.DOTALL)
            ...
            while m:
                ...
                while i < len(sql) and depth:
                    ...
                    i += 1
                ctes[cte_name] = sql[start:i-1].strip()
                # remove this CTE header
                sql = sql[i:].lstrip(",").strip()
                m   = re.match(r"(?i)(\w+)\s+AS\s*\(", sql)
                if m:
                    # chained CTE
                    cte_name = m.group(1)
                    start    = m.end()
    ...
    def _translate_select(sql, df_map, result_var="result_df"):
        ...
        # DISTINCT
        distinct = bool(re.search(r"(?i)\bSELECT\s+DISTINCT\b", sql))
        # TOP N
        top_m = re.search(r"(?i)\bSELECT\s+TOP\s+(\d+)\b", sql)
        top_n = int(top_m.group(1)) if top_m else None

        # Strip SELECT ... FROM  columns
        sel_m = re.search(r"(?i)\bSELECT\b\s+(?:DISTINCT\s+)?(?:TOP\s+\d+\s+)?(.*?)\s+\bFROM\b", sql, re.DOTALL)
        cols_part = sel_m.group(1).strip() if sel_m else "*"

        # FROM clause (main table)
        from_m = re.search(r"(?i)\bFROM\b\s+([\w\.]+)(?:\s+(?:AS\s+)?(\w+))?", sql)
        main_tbl = ""
        main_alias = ""
        if from_m:
        ...
        join_pat = re.compile(
            ...
            re.DOTALL)
        joins = list(join_pat.finditer(sql))

        # WHERE
        where_m = re.search(r"(?i)\bWHERE\b(.*?)(?=\s+GROUP\s+BY|\s+HAVING|\s+ORDER\s+BY|\s+LIMIT|$)", sql, re.DOTALL)
        where_str = where_m.group(1).strip() if where_m else ""

        # GROUP BY
        grp_m = re.search(r"(?i)\bGROUP\s+BY\b(.*?)(?=\s+HAVING|\s+ORDER\s+BY|$)", sql, re.DOTALL)
        grp_cols = [_clean_name(c.strip()) for c in grp_m.group(1).split(",")] if grp_m else []

        # HAVING
        hav_m = re.search(r"(?i)\bHAVING\b(.*?)(?=\s+ORDER\s+BY|$)", sql, re.DOTALL)
        having_str = hav_m.group(1).strip() if hav_m else ""

        # ORDER BY
        ord_m = re.search(r"(?i)\bORDER\s+BY\b(.*?)(?=\s+LIMIT|$)", sql, re.DOTALL)
        ord_cols = []
        if ord_m:
        ...
        # JOINs
        for jm in joins:
            j_type  = _join_type(re.sub(r"\s+", " ", (jm.group(1) or "INNER ")).split()[0])
            j_tbl   = _clean_name(jm.group(2))
            j_alias = jm.group(3) or j_tbl
            on_cond = jm.group(4).strip()
            ...
            if j_tbl not in df_map:
                ...
                df_map[j_tbl] = j_var
            on_expr  = on_cond.replace('"', "'")
            new_var  = f"joined_{j_tbl.lower()}_df"
            lines.append(f'{new_var} = {chain_var}.join({j_var}, F.expr("{on_expr}"), "{j_type}")')
            chain_var = new_var

        # SELECT columns / aggregates
        raw_cols  = _parse_select_cols(cols_part)
        is_star   = cols_part.strip() == "*"
        agg_funcs = [f for f in raw_cols if re.match(r"(?i)(SUM|COUNT|AVG|MIN|MAX|STDEV|VAR)\s*\(", f.strip())]
    ...
    def _translate_insert(sql, target_var="result_df"):
        """INSERT INTO table SELECT … → .write.saveAsTable()."""
        tgt_m = re.search(r"(?i)INSERT\s+(?:INTO\s+)?([\w\.]+)", sql)
        tgt   = _clean_name(tgt_m.group(1)) if tgt_m else "target_table"
        sel_sql = re.sub(r"(?i)INSERT\s+(?:INTO\s+)?[\w\.]+[^S]*", "", sql, count=1).strip()
    ...
    has_cte    = bool(re.search(r"(?i)\bWITH\b\s+\w+\s+AS\s*\(", body))
    ...
    if has_cursor: auto_notes.append("CURSOR → DataFrame collect() + Python loop stub")
    ...
    if has_cursor:
        lines += [
            "# ── NOTE: CURSOR replaced with DataFrame collect() ──────────────────────",
            "# cursor_rows = source_df.collect()          # Only for small datasets!",
            "# for row in cursor_rows:",
            "#     value = row['column_name']",
            "#     # ... process each row ...",
            "# Prefer: df.transform() / groupBy().applyInPandas() for large data",
            "",
        ]
    ...
    for stmt in stmts:
        ...
        if re.match(r"(?i)DECLARE\s+\{", stmt) or re.match(r"(?i)DECLARE\s+@", stmt):
            var_m = re.match(r"(?i)DECLARE\s+[@{](\w+)\}?\s+([\w()]+)(?:\s*=\s*(.+))?", stmt)
            if var_m:
                vname, vtype, vdef = var_m.group(1), var_m.group(2), var_m.group(3) or "None"
                py_type = {"INT":"int","BIGINT":"int","VARCHAR":"str","NVARCHAR":"str",
                           "BIT":"bool","FLOAT":"float","DATETIME":"str"}.get(vtype.upper(),"str")
        ...
        if re.match(r"(?i)UPDATE\b", stmt):
            ...
            if tgt_m:
                ...
                _set_clean = re.sub(chr(91) + r"(\w+)" + chr(93), r"\1", set_p)
                lines += [
                    f"# ── UPDATE {tgt} ──────────────────────────────────────────────────────",
