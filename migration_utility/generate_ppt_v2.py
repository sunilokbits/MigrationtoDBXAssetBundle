...
Full-featured: 14 UI tabs, 85+ API endpoints, Self-Healing, CDC/DLT, etc.
"""
...
box(sl, Inches(0), Inches(0.55), Inches(13.333), Inches(0.2), RGBColor(0xE3, 0xF2, 0xFD),
    "14 UI Pages  •  85+ API Endpoints  •  Metadata-Driven Medallion  •  CDC/DLT  •  Self-Healing  •  Auto Infrastructure  •  Data Quality  •  Reconciliation",
...
box(sl, dbr_x + Inches(0.12), nb_y, dbr_w - Inches(0.24), Inches(0.22),
    TEAL, "Auto-Generated Notebooks  (Standard PySpark  +  DLT Mode  +  CDC)", sz=7.5, fc=WHITE, bold=True)

notebooks = [
    ("00_Orchestrator",    "Chains stages\nDLT or Spark"),
    ("01_Extract",         "JDBC + Watermark\nParquet → Volumes"),
    ("02_Bronze/DLT",      "Landing → Bronze\nAudit + DQ checks"),
...
box(sl, Inches(1.13), row2_y + Inches(1.15), Inches(0.97), Inches(0.22), TEAL,
    "CDC / DLT Engine", sz=6.5, fc=WHITE, bold=True)
mbox(sl, Inches(1.13), row2_y + Inches(1.39), Inches(0.97), Inches(0.45),
     ["Change Tracking CDC", "DLT Auto Loader",
...
tech_items = [
    ...
    ("Delta Live Tables",      "DLT Auto Loader + expectations + CDC"),
...
mbox(sl, flow_x + Inches(0.1), fs_y + Inches(0.52), Inches(5.3), Inches(0.32),
     ["Standard Mode: Extract → Bronze → Silver (3 stages)  |  DLT Mode: Extract → DLT Bronze+Silver (2 stages)",
...
txt(sl, Inches(0.2), Inches(7.2), Inches(13), Inches(0.2),
    "SQL → Databricks Migration Studio v2.0  |  Metadata-Driven Medallion  |  CDC + DLT  |  Self-Healing  |  14 UI Pages  |  85+ Endpoints  |  deployconfig.json",
...
feature_card(sl2, col2_x, cy2, col2_w, Inches(0.78),
    ...
    ["Change Tracking CDC mode for incremental loads",
     "DLT Auto Loader with Delta expectations",
...
before_after = [
    ...
    ("CDC/DLT",        "Basic watermark","CDC + DLT + Auto Loader"),
...
pages = [
    ...
    ("🏗️ Medallion Arch", LANDING_CLR, RGBColor(0xEC, 0xEF, 0xF1),
     ["Generate 6 notebooks", "Standard + DLT mode", "Multi-catalog support",
...
extra_pages = [
    ("⚙️ Settings", DARK_GRAY, LIGHT_GRAY,
     ["Databricks host + token", "Source DB connection + test", "Metadata catalog/schema",
      "CDC/DLT mode config", "Auto-saves to deployconfig.json"]),
