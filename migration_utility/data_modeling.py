...
def generate_er_json(model: dict) -> dict:
    ...
    return {"nodes": nodes, "edges": edges, "schema_type": model["schema_type"]}


def generate_ddl(model: dict, catalog: str = "main", schema: str = "default") -> str:
    """Generate Databricks Delta DDL for all tables in the model."""
    lines = []
    lines.append(f"-- ═══════════════════════════════════════════════════════")
    ...
    for tbl in model["dimensions"] + model["facts"]:
        ...
        for c in tbl["columns"]:
            ...
            if c.get("is_pk"):
                ...
                comment = f" -- FK → {c['fk_table']}"
            col_defs.append(f"  {c['name']} {dtype}{nullable}{comment}")

        lines.append(",\n".join(col_defs))
        lines.append(f") USING DELTA")
        lines.append(f"COMMENT '{role.upper()} table: {tname}';\n")

    # Add ALTER TABLE for FK constraints (informational — Delta supports CONSTRAINT)
    for r in model["relationships"]:
        fk_col = _guess_fk_column(r["from"], r["to"], model)
        if fk_col:
            cname = f"fk_{r['from']}_{r['to']}"[:60]
            lines.append(
                f"ALTER TABLE {catalog}.{schema}.{r['from']} "
                f"ADD CONSTRAINT {cname} FOREIGN KEY ({fk_col}) "
                f"REFERENCES {catalog}.{schema}.{r['to']};"
            )
...
def apply_manual_edits(model: dict, edits: dict) -> dict:
    ...
    for ra in edits.get("relationship_adds", []):
        if not any(r["from"] == ra["from"] and r["to"] == ra["to"] for r in model["relationships"]):
            model["relationships"].append({"from": ra["from"], "to": ra["to"], "type": ra.get("type", "many-to-one")})
...
def _edit_column(model: dict, table_name: str, col_name: str, changes: dict):
    """Apply column edits (rename, retype, nullable)."""
    for tbl in model["facts"] + model["dimensions"]:
