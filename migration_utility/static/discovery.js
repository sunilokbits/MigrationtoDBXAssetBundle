...
function _discReadSourceConfig(){
  ...
  return {
    source_type: pick('cfgSrcType','srcType','wfSrcType') || 'sqlserver',
    server:      pick('cfgSrcServer','srcServer','wfSrcServer'),
    database:    pick('cfgSrcDb','srcDb','wfSrcDb'),
...
/* Supported source systems for live Discovery scan. The backend
   ...
   sources are supported for live scans today. */
const _DISC_LIVE_SUPPORTED = new Set(['azuresql','sqlserver','synapse']);
...
function _discUpdateSourceBadge(){
  ...
  const label = ({azuresql:'Azure SQL',sqlserver:'SQL Server',synapse:'Synapse',mysql:'MySQL',postgresql:'PostgreSQL',oracle:'Oracle'})[cfg.source_type] || cfg.source_type;
  const srv = cfg.server ? (cfg.server.length>32?cfg.server.slice(0,32)+'…':cfg.server) : '<not set>';
  const db  = cfg.database || '<no db>';
  const ok  = _DISC_LIVE_SUPPORTED.has(cfg.source_type) && cfg.server && cfg.database;
  const warn = !_DISC_LIVE_SUPPORTED.has(cfg.source_type)
    ? ' ⚠ live scan supports Azure SQL / SQL Server / Synapse only'
...
async function discRunScan(){
  ...
  if(srcType === 'live' || srcType === 'both'){
    ...
    if(!_DISC_LIVE_SUPPORTED.has(cfg.source_type)){
      ...
      _discToast('Live Discovery scan currently supports Azure SQL, SQL Server, and Synapse only. Your Settings source is "'+cfg.source_type+'". Switch to Static Demo Objects to preview.', 'err');
      return;
    }
    if(!cfg.server || !cfg.database){
      ...
      _discToast('Set Server and Database in Settings → Source Database first.', 'err');
      return;
    }
...
function discShowRCA(name){
  const panel = G('discRcaPanel');
  const overlay = G('discRcaOverlay');
  if(!panel || !_discReport) return;
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
function discInit(){
  // If we have cached results, re-render
...
function _discRenderProfileTableList(){
  ...
  _discProfileTables.forEach((t,i)=>{
    ...
    html += '<tr style="border-bottom:1px solid var(--border);cursor:pointer;" onmouseover="this.style.background=\'rgba(236,72,153,.05)\'" onmouseout="this.style.background=\'transparent\'" onclick="discProfileOpen(\''+tesc+'\')">'+
      '<td style="padding:8px 10px;font-family:\'SF Mono\',Consolas,monospace;color:var(--t1);font-weight:600;">'+t.name+'</td>'+
      ...
      '<td style="padding:8px 10px;text-align:center;"><button class="btn btn-ghost btn-xs" style="font-size:10px;color:#EC4899;border:1px solid #EC489944;" onclick="event.stopPropagation();discProfileOpen(\''+tesc+'\')">📊 Profile</button></td>'+
      '</tr>';
  });
  html += '</tbody></table>';
  box.innerHTML = html;
...
async function discProfileOpen(tableName){
