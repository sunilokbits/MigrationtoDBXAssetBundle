...
#  SCAN: LIVE SOURCE DB
# ─────────────────────────────────────────────────────────────────────────────
def scan_live_source(source_config: dict) -> list:
    """
    Connect to a live SQL Server / Azure SQL and discover + analyse all objects.
    source_config: {source_type, server, database, username, password}
    ...
    for row in cursor.fetchall():
        objects.append(('stored_procedure', row[1], row[3]))
    ...
    """)
    for row in cursor.fetchall():
        objects.append(('view', row[1], row[2]))
    ...
    """)
    for row in cursor.fetchall():
        objects.append(('udf', row[1], row[2]))

    # Tables — fetch DDL-like metadata for each user table
    table_meta = []  # (name, col_count, row_count, has_triggers, idx_count, fk_count, chk_count, ddl_sketch)
    try:
        cursor.execute("""
            SELECT t.name,
        ...
        for row in cursor.fetchall():
            tbl_name = row[0]
            col_count = row[1] or 0
            tbl_row_count = int(row[2]) if row[2] else 0
            trig = bool(row[3])
            idx_cnt = row[4] or 0
            fk_cnt = row[5] or 0
            chk_cnt = row[6] or 0
            ...
            try:
                ...
                pass
            ddl_sketch = f'CREATE TABLE [{tbl_name}] (\n' + ',\n'.join(col_rows) + '\n)' if col_rows else ''
            table_meta.append((tbl_name, col_count, tbl_row_count, trig, idx_cnt, fk_cnt, chk_cnt, ddl_sketch))
    except Exception:
        pass
    ...
    for obj_type, obj_name, obj_code in objects:
        ...
        # Enrich with live dependency data
        if obj_name in dep_map:
            extra_refs = [r for r in dep_map[obj_name] if r.lower() != obj_name.lower()]
    ...
    # Analyse tables
    for tbl_name, col_cnt, tbl_rows, trig, idx_cnt, fk_cnt, chk_cnt, ddl in table_meta:
        a = analyse_table_object(
            name=tbl_name, ddl=ddl,
...
def generate_bom_csv(analyses: list) -> str:
    """Generate a CSV string for Bill of Materials export."""
    output = io.StringIO()
