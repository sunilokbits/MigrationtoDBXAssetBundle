...
def _build_sql_conn_str(source_type, server, database, username, password):
    ...
    base = f"DRIVER={{{driver}}};SERVER={server};DATABASE={database};UID={username};PWD={password}"
    is_v18 = "18" in driver
...
def migrate_start():
    ...
    try:
        ...
        def _bg():
            migrator.migrate_tables_parallel(tables, warehouse_id, job_id, max_workers)
...
def medallion_run_pipeline():
    ...
    try:
        ...
        result = connector.run_notebook(
            ...
            params={"load_type": load_type, "password": password},
        )
        return jsonify(result)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
